...
use crate::{
    Metadata, Parameter, Parameters, Version,
...
pub(super) struct WritableWidget<'a> {
    parameters: &'a mut Parameters,
    options: MetadataOptions,
}

impl<'a> WritableWidget<'a> {
    pub(super) fn new(parameters: &'a mut Parameters, options: MetadataOptions) -> Self {
        Self {
            parameters,
            options,
        }
    ...
    fn parameters(&mut self, ui: &mut Ui, name: &str) {
            ...
            .filter(|parameter| parameter.name == name)
        {
            ui.vertical(|ui| {
                ...
                ui.label(ui.localize(&format!("{PREFIX}_{name}")));
            });
            ui.vertical(|ui| match name {
                NAME => name_value(&mut parameter.name, ui),
                VERSION => versions_value(parameter.value.get_or_insert_default(), ui),
                _ => {}
            });
            ui.end_row();
        }
    ...
    pub(super) fn show(&mut self, ui: &mut Ui) {
        ...
        Grid::new("WritableGrid").num_columns(2).show(ui, |ui| {
            ...
            if self.options.name {
                ...
                {
                    key_value(ui, NAME, |ui| name_value(&mut parameter.name, ui));
                }
            }
            // Description
            if self.options.description {
                if let Some(parameter) = self
                    .parameters
                    .iter()
                    .find(|parameter| parameter.name == DESCRIPTION)
                {
                    key_value(ui, DESCRIPTION, |ui| description(&mut parameter.name, ui));
                }
            ...
            if self.options.versions {
                key_value(ui, VERSIONS, |ui| {
                    versions_value(&mut self.metadata.versions, ui)
                });
            }
            ui.separator();
            ui.separator();
...
impl Writable<'_> {
    pub(super) fn show(&mut self, ui: &mut Ui) {
        ...
        Grid::new("WritableGrid").num_columns(2).show(ui, |ui| {
            // Name
            if self.options.name {
                key_value(ui, NAME, |ui| name_value(&mut self.metadata.name, ui));
            }
            // Description
            if self.options.description {
                key_value(ui, DESCRIPTION, |ui| {
...
fn key_value(ui: &mut Ui, name: &str, value: impl FnOnce(&mut Ui)) {
    ui.vertical(|ui| {
        ui.style_mut().wrap_mode = Some(TextWrapMode::Extend);
        ui.label(ui.localize(&format!("{PREFIX}_{name}")));
    });
...
fn name_value(name: &mut String, ui: &mut Ui) {
...
fn versions_value(versions: &mut Vec<Version>, ui: &mut Ui) {
