        ...
        self.parameters.retain_mut(|Parameter { name, value }| {
            ...
            ui.horizontal(|ui| {
                ...
                if let Some(value) = value {
                    match &**name {
                        DATE => {
                            // Пытаемся распарсить дату из строки, иначе берем текущую
                        ...
                        // VERSION | IDENTIFIER => {
                        IDENTIFIER => {
                            // Используем f64, чтобы поддерживать версии вида "1.5" (или замените на i64 для целых чисел)
                            let mut num = value.parse::<u64>().unwrap_or(0);

                            if ui.add(DragValue::new(&mut num).speed(1)).changed() {
                                *value = num.to_string();
                            }
                        }
                        DESCRIPTION => {
